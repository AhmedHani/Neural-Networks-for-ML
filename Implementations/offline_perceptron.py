...
# Batch size for online learning (default: 100)
...
# Model log data
print("Model: " + str("Offline Perceptron"))
...
# Store avg. error for each batch
error = 0.0
...
for epoch in range(0, FLAGS.epochs):
    for i in range(0, len(train_features)):
        ...
        # Accumulate the error by subtracting the desired output by the actual output
        error += desired_output - current_output

        # If batch ends
        if i % FLAGS.batch_size:
            # Get avg. error
            error = (error * 1.0) / (FLAGS.batch_size * 1.0)
            ...
            # Reinitialize the error for the next batch
            error = 0.0
